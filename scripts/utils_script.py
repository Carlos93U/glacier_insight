...
def reports_graphs(mountain_range_id, mountain_name, cropped_image_1989, cropped_image_2020, green_border_only, glacier_1989_area, glacier_2020_area, mask_filled):
    """
    Generates a graphical report comparing the mountain range in 1989 and 2020, highlighting glacier areas.
    
    Parameters:
    mountain_range_id (str): ID of the mountain range.
    ...
    # to save
    path_to_save = "./data/raw/" + mountain_range_id + f"_{mountain_range[mountain_range_id]}/Glaciers_comparison.png"
...
def draw_histograms(mountain_range_id, glacier_1989_area, glacier_2020_area, mask_filled):
    """
    ...
    glacier_2020_area (numpy.ndarray): Masked image showing glacier area in 2020.
    mask_filled (numpy.ndarray): Binary mask where the largest contour is filled.
    mountain_range (dict): Dictionary containing the mapping of mountain range IDs to names.
    """
    ...
    path_to_save = f"./data/raw/{mountain_range_id}_{mountain_range[mountain_range_id]}/histograms_comparison.png"
    plt.savefig(path_to_save, dpi=300)
    
    # Save results to results.txt
    results_path = f"./data/raw/{mountain_range_id}_{mountain_range[mountain_range_id]}/results.txt"
    with open(results_path, "w") as f:
        f.write(f"Glacier Pixels in 1989: {total_pixels_1989}\n")
        f.write(f"Glacier Pixels in 2020: {total_pixels_2020}\n")
        f.write(f"Percentage change in glacier area: {percentage_change} %\n")
